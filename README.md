...
- `preview: string | null;` - хранит id выбранной карточки.
...
- `setPreview(item: IItem): void;` - записывает id в поле preview.
...
- `card:deleteFromBasketView` - в модальном окне подробного описания кликом по кнопке товар удаляется из корзины, если ранее был добавлен в неё.
- `card:deleteFromBasket` - в корзине кликом по кнопке удаления происходит удаление карточки из корзины.
